...
def add_worker():
    ...
    if request.method == "POST":
        ...
        if is_postgres():
            ...
            # Workers table uses INTEGER PRIMARY KEY
            # so generate the next ID manually.
            id_row = fetch_one("""
...
def report_pdf():
    ...
    for row in data:
        ...
        table_data.append([
            ...
            f"{safe_float(row_value(row, 'basic_salary', 0)):.2f}",

            f"{safe_float(row_value(row, 'present_days', 0)):.1f}",

            f"{safe_float(row_value(row, 'absent_days', 0)):.1f}",

            f"{safe_float(row_value(row, 'ot_hours', 0)):.1f}",

            f"{safe_float(row_value(row, 'ot_amount', 0)):.2f}",

            f"{safe_float(row_value(row, 'refreshment', 0)):.2f}",

            f"{safe_float(row_value(row, 'advance', 0)):.2f}",

            f"{safe_float(row_value(row, 'net_salary', 0)):.2f}"
        ])
